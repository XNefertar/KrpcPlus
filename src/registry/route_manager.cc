...
std::vector<std::string> RouteManager::GetRouteNodes(
  ...
  std::shared_ptr<const RouteTable> local_table =
      std::atomic_load(&_curRouteTable);
...
void RouteManager::UpdateRouteTable(const std::string& path,
                                     ZookeeperClient* client, bool watch) {
  LOG(INFO) << "Updating route for " << path;
  if (!client) {
    LOG(ERROR) << "ZookeeperClient is null";
    return;
  }
  ...
  std::atomic_store(&_curRouteTable,
                    std::shared_ptr<const RouteTable>(newTable));
}
